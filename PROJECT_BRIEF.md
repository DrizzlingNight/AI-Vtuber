...
Phase 6 的第一版不需要程式自動控制 OBS；由操作者手動啟動並套用固定設定即可。最小
整合需要：

- 讓 OBS 擷取 VTube Studio 畫面。
- 讓 OBS 擷取 TTS 播放裝置或 TTS 專用音訊來源。
- 用本地文字檔更新字幕來源。
- 使用預定直播的解析度、FPS、編碼器與 bitrate，先做本機錄影測試。

Phase 7 再加入：

- 以 obs-websocket 自動切換場景與更新字幕。
- 訂閱、Raid 或特殊事件的字幕與動畫。
- TTS、背景音樂與遊戲音效分軌。
- 故障畫面或離線待機畫面。
...
- 可以在測試頻道連續運作至少一小時；這是核心互動鏈路實機驗收，不啟動 OBS 或直播。
- 沒有訊息回圈、佇列無限增長或模型卡在某個表情。
- 單一元件重啟不需要整套程式人工重開。

### Phase 6：安全、最小 OBS 整合與直播穩定性
...
- 由操作者手動啟動 OBS，固定擷取 VTube Studio、TTS 音訊與本地字幕；本階段不要求
  程式自動操作 OBS。
- 先以預定直播的解析度、FPS、編碼器與 bitrate 完成本機錄影，確認畫面構圖、聲音、
  字幕與編碼穩定。
- 只有在使用者明確授權開台後，才執行受控 Twitch 直播；量測 OBS／網路掉幀、bitrate、
  重新連線，以及第二台裝置實際看到的影音與字幕。
- 在包含 OBS 與直播輸出的完整環境進行 4～8 小時 soak test。
...
- obs-websocket 場景、字幕、待機與故障畫面自動化。
- 獨立 Twitch bot 帳號。
...
10. Phase 5 至少完成一小時核心互動鏈路實機測試；這不包含 OBS，也不是直播穩定性證據。
11. Phase 6 先通過 OBS 本機錄影，再經使用者明確授權完成受控直播與 4～8 小時完整環境
    soak test，之後才進行公開直播。
